...
func (wrapper *SerialMediaWrapper) PlayChanges(ctx context.Context, target string) (*rpc.Subscription, error) {
	wrapper.OpLock.RLock()
	defer wrapper.OpLock.RUnlock()
	...
	channelContext, channelContextCancel := context.WithCancel(context.Background())
	incomingChannel := connection.PlayChannel(channelContext)
	sub := notifier.CreateSubscription()
